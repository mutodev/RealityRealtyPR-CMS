<?php
    ...
    class ListMax_Module_Brokers_Collection extends ListMax_Core_Collection{
        ...
        protected function filter( &$select ){        
            ...
            //Letter
            if( $this->isFilteredBy( "letter" ) ){
                $letter  = $this->getFilterBy("letter");
                $letters = preg_split( "/[\s,]+/" , $letter , -1 , PREG_SPLIT_NO_EMPTY );
                foreach( $letters as $k => $v ){
                    $letters[ $k ] = "'".mysql_escape_string(strtoupper($v))."'";
                }
                $letters = implode(" , " , $letters );
                $select->where(" UPPER( SUBSTRING(Usuarios.apellido,1,1))  IN( $letters )");
            }
            ...
            //Basic
            if( $this->isFilteredBy( "basic" ) ){
                $select->where("Usuarios.id IN     ( SELECT group_user.id_user FROM group_user WHERE id_group IN( 5  ) )" );
            }

            //Standard
            if( $this->isFilteredBy( "standard" ) ){
                $select->where("Usuarios.id IN     ( SELECT group_user.id_user FROM group_user WHERE id_group IN( 7 ) )" );
            }
