YAMMON.Widgets.DatePicker = new Class({
    ...
    initialize: function( node , options ){
        ...
        if ( render == 'single' )
            this.renderSingle( node, options );
        else
            this.renderSplitted( node, options );
    },
    renderSingle: function( node, options ){

        var id       = node.get('id');
        var prefix   = id.substring( 0 , id.length - 2 );

        var calendar = new CalendarEightysix( prefix , {
            'linkWithInput': true ,
            'format':  options.format,
            'preserveTimeOnPick': true,
            'prefill': false ,
            'toggler': node ,
            'offsetX': 0
        });
    },
    renderSplitted: function( node, options ){

        var id       = node.get('id');
        var prefix   = id.substring( 0 , id.length - 2 );
        var full_el  = $(prefix);
        var month_el = $(prefix + '_m');
        var day_el   = $(prefix + '_d');
        var year_el  = $(prefix + '_y');
        var calendar = new CalendarEightysix( node , {
            'linkWithInput': false ,
            'format':  options.format,
            'preserveTimeOnPick': true,
            'prefill': false ,
        ...
        var update_date = function() {
            ...
            var month = month_el ? month_el.get('value') : null;
            var day   = day_el   ? day_el.get('value')   : null;
            var year  = year_el  ? year_el.get('value')  : null;
        ...
        calendar.addEvent('change', function(date) {
            ...
            if( month_el )
            	month_el.set('value', date.get('month') + 1);
            if( day_el )
        	    day_el.set('value', date.get('date'));
            if( year_el )
            	year_el.set('value', date.get('year'));

        });

        if( month_el )
            month_el.addEvent('change', update_date);

        if( day_el )
            day_el.addEvent('change', update_date);

        if( year_el )
            year_el.addEvent('change', update_date);
...
var CalendarEightysix = new Class({
	...
	initialize: function(target, options) {
		...
		if ( $defined(this.options.toggler) && this.options.toggler != this.target ) {
	...
	pick: function(e) {
        ...
        //Preserve Time on Pick
        if (this.options.preserveTimeOnPick && this.target.get('value')) {
            var oldValue = new Date().parse(this.target.get('value'));
        ...
        //Format
		value = this.format(this.selectedDate);
