...
class Helper_Form_Element_Photos extends Helper_Form_Element_Valued
{
    ...
    public function renderContent( $value = array() )
    {
        ...
        $approved_dom_name = $dom_name."[is_approved][]";
        ...
        if (Auth::hasPermission('company.manage')) {
            $html->open('div' , array('class' => 'ym-form-approved' )  );
            $html->text( t('Approved') );
            $html->close('div');

            $html->open("select", array('name' => $approved_dom_name, 'class' => 'ym-form-select'));
            foreach( [0 => t('No'), 1 => t('Yes')] as $k => $v ){

                $atts             = array();
                $atts["value"]    = $k;

                if(@$value['is_approved'] == $k )
