...
class Gettext
{
    ...
    */
    ...
    function &factory($format, $file = '')
    {
        ...
        $path   = dirname( __FILE__ ). DIRECTORY_SEPARATOR . 'Gettext' . DIRECTORY_SEPARATOR;
        if (!@include_once $path . $format . '.php') {
            return File_Gettext::raiseError($php_errormsg);
        }
        $class = 'Gettext_' . $format;
    ...
    {
        if (!is_file($pofile)) {
            return Gettext::raiseError("File $pofile doesn't exist.");
        }

        $path   = dirname( __FILE__ ). DIRECTORY_SEPARATOR . 'Gettext' . DIRECTORY_SEPARATOR;        
        include_once $path.'PO.php';
